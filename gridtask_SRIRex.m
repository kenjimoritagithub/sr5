function Out = gridtask_SRIRex(a_SR,a_IR,b,g,decay_rate,c,dur_ini,dur_epoch,num_epoch,R_prob)
...
w = zeros(25,2); % weights for SR-based system-specific values for [D1, D2] pathways
IRSV = zeros(25,2); % IR-based system-specific values for [D1, D2] pathways
I25 = eye(25);
...
% main loop
G_times = [];
TDRPEseq = NaN(dur_ini+num_epoch*dur_epoch,2);
nextS = 1; % next state
for k = 1:dur_ini+num_epoch*dur_epoch
    
    % decay/forgetting
    w = w * (1 - decay_rate);
    IRSV = IRSV * (1 - decay_rate);
    ...
    % TD-RPE ([SR-value-specific, IR-value-specific])
    if currS ~= G
        TDRPE = R(currS) + [g*sum(SR(nextS,:)*w) - sum(SR(currS,:)*w), g*sum(IRSV(nextS,:)) - sum(IRSV(currS,:))];
    else
        TDRPE = R(currS) + 0 - [sum(SR(currS,:)*w), sum(IRSV(currS,:))];
    end
    TDRPEseq(k,:) = TDRPE;
    
    % update of IRSV
    TDRPE_IRreceived = (TDRPE(1) + (1+c)*TDRPE(2))/(2+c);
    if TDRPE_IRreceived >= 0
        IRSV(currS,1) = IRSV(currS,1) + a_IR(1)*TDRPE_IRreceived;
    else
        IRSV(currS,2) = IRSV(currS,2) + a_IR(2)*TDRPE_IRreceived;
    end
    
    % update of w
    TDRPE_SRreceived = ((1+c)*TDRPE(1) + TDRPE(2))/(2+c);
    if TDRPE_SRreceived >= 0
        w(:,1) = w(:,1) + a_SR(1)*SR(currS,:)'*TDRPE_SRreceived;
    else
        w(:,2) = w(:,2) + a_SR(2)*SR(currS,:)'*TDRPE_SRreceived;
    end
