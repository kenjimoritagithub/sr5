...
save data_figure9A totalRset
for k_R_prob = 1:length(R_prob_set)
    heatmap_performance(mean(totalRset{k_R_prob},3),length(a_posiprop_set),[],['Figure9A-' num2str(k_R_prob)]);
end
...
save data_figure9B totalRset
for k_dur_reset = 1:length(dur_reset_set)
    heatmap_performance(mean(totalRset{k_dur_reset},3),length(a_posiprop_set),[],['Figure9B-' num2str(k_dur_reset)]);
end
...
save data_figure9CD totalRset
%
tmp_CD = 'CD';
for k_cond = 1:2
    heatmap_performance(mean(totalRset{k_cond},3),length(a_posiprop_set),[],['Figure9' tmp_CD(k_cond)]);
end
...
save data_figure9E totalRset
heatmap_performance(mean(totalRset,3),length(a_posiprop_set),[],'Figure9E');
...
save data_figure9F totalRset
heatmap_performance(mean(totalRset,3),length(a_posiprop_set),[],'Figure9F');
